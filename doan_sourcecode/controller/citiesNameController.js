...
exports.getCitiesName = catchAsync(async (req, res, next) => {
  ...
  const province = Province.find({}, {_id: 1, id: 1, provincename: 1, realname: 1});

  const data = await Promise.all([results, province])

  const provinceData = data[1].reduce((acc, cur) => Object.assign(acc, { [cur.id]: cur._id, [cur.provincename]: cur.realname }), {});
  ...
  const cities = data[0].map(row => {
    let city = {
      name: row.adm1_name,
      realname: provinceData[row.adm1_name],
      id: row.id,
      lat: row.lat,
      lng: row.lng,
